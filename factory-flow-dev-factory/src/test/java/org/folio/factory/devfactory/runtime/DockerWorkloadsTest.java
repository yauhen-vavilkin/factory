...
class DockerWorkloadsTest {
    ...
    void privateWorkloadRequiresStopBeforeExportAndIsRemoved() throws Exception {
        ...
        try (var workload = new DockerWorkloads().create("maven:3.9-eclipse-temurin-21", source)) {
            ...
            assertThat(workload.execute(List.of("sh", "-c", "test ! -e /var/run/docker.sock && test -z \"$GH_TOKEN$GITHUB_TOKEN$JIRA_API_TOKEN\" && test -f /workspace/source.txt && printf changed > /workspace/source.txt"), 30).exitCode()).isZero();
            workload.stop();
