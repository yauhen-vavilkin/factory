...
**Concrete topology:** one per-run trusted gateway container, managed by Factory with the sandbox. Coding attaches only to a private internal network with this gateway. Gateway also has outward connectivity. No gateway port is published on the host, no DB/control API is attached to this network, and no routing/NAT forwarding is enabled through the gateway. Use the existing Java distribution for a narrow gateway process where practical; no new general-purpose service framework is justified.
