...
- **Model upstream:** the coding sandbox always requests provider `openai-compatible` and model alias `factory-coding` from the local coding gateway. The gateway replaces the alias with `FACTORY_MODEL_ID` and sends the request only to `FACTORY_MODEL_BASE_URL` (HTTPS, no credentials in the URL, no redirects) with `FACTORY_MODEL_API_KEY`. These three values live only in the gitignored `.env`; use the endpoint that matches your credential, because the system does not convert between endpoints.
